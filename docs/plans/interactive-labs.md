...
- **回放主线**：**外循环 `j` 遍历 K/V 块，内循环 `i` 遍历 Q 块**（V1 的顺序；V2 才调换成 Q 外 K 内，见 L07）。每轮：载入 `K_j, V_j` → `S_ij = Q_i K_j^T` → 行最大值 → `P_ij = exp(S_ij - m_new)` → 用修正因子更新 `ℓ_i, O_i` → **`S, P` 从不落回 HBM**。

  > ⚠️ **本节此前把顺序写反了**（写成 Q 外 K 内），L06 实现时被 agent 查教程发现并纠正。依据：`6.1-FlashAttention V1详解.md:280,451` 明确 V1 是 K/V 外层，且第 451 行指出「V1 IO 较多的根本原因正是这个顺序」；`6.2` 第 69 行说 V2「将循环顺序**调换**为外循环遍历 Q 块」。**这个顺序是 L06/L07 的分界本身** —— 若 L06 用了 V2 的顺序，L07 就没有 diff 可做了。
